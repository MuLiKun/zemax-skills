...
- 支持普通标准模板模式：GUI/CLI 可直接选择产品类型（RX/TX）、模板、公差等级、MC 次数、保存数量、补偿方式与是否保存 WC/BC；当前 TX 暂复用 RX 模板；默认自动使用主波长、自动识别公差结束面，并启用模板级视场映射（`标准分析` 目标 `0,0.5,0.9,-0.9`，`完整视场分析` 目标 `0,-0.25,0.25,-0.5,0.5,-0.7,0.7,-0.9,0.9,-1,1`）；运行期临时标准配置使用唯一文件名，避免覆盖用户导出的同名配置；标准模板公差向导仅保留面倾斜/元件偏心/元件倾斜（不含面偏心），并自动追加中心指向偏移（`POINTING_DY_F0_mm`/`POINTING_DX_F0_mm`）与焦距偏移百分比（`EFL_DELTA_PCT`）评价项
